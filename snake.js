window.onload = function()
{
	...
	ctx.fillRect(2, 20, canvas.width - 4, canvas.height - 24);
    ...
    {
	//drawGame();
	active = false; // spacebar
	alert('Press Enter to Resume!');
	//confirm("Press O.K. to Resume!");
	//ctx.fillStyle = 'Green';
	//ctx.fillText('Press Enter to\nResume!', ((canvas.width / 2) - (ctx.measureText('Your Score Was: ' + score).width / 2)), 260);
    }
    else if (e.keyCode === 13 || e.keyCode === 16)
    {
	active = true; // Enter shift doesn't working keyCode = 16
	drawGame();
    }});
